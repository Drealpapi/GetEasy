...
export default function LocationSearchScreen({ navigation }: any) {
  ...
  const popularLocations = POPULAR_CITIES.map(loc => ({ ...loc, count: 15 }));
  ...
  const handleSearchQueryChange = (text: string) => {
    ...
    if (text.length > 1) {
      ...
      const citiesResults: { state: string; city: string }[] = [];
      results.forEach(r => {
        r.cities.forEach(city => {
          citiesResults.push({ state: r.state, city });
        });
      });
      setSearchSuggestions({ states, cities: citiesResults });
    } else {
  ...
  const renderService = ({ item }: { item: Service }) => (
    ...
    >
      ...
      <View style={styles.serviceFooter}>
        <Text style={styles.serviceLocation}>📍 {item.city}, {item.state}</Text>
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.activeFilters}>
          ...
          {selectedCity && (
            <View style={styles.filterBadge}>
              <Text style={styles.filterBadgeText}>🏙️ {selectedCity}</Text>
            </View>
          )}
          {selectedCategory && (
            <View style={styles.filterBadge}>
